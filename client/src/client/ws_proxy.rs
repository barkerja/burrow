...
pub struct WebSocketProxy {
    ...
    local_ws: Arc<Mutex<futures_util::stream::SplitSink<WsStream, Message>>>,
...
impl WebSocketProxy {
        ...
        for header in &headers {
            if header.len() >= 2 {
                ...
                if name_lower == "cookie" || name_lower == "authorization" {
                    if let Ok(value) = header[1].parse() {
    ...
    pub async fn close(&self, _code: u16, _reason: &str) {
        let mut ws = self.local_ws.lock().await;
        let _ = ws.send(Message::Close(None)).await;
    }

    /// Run the proxy, forwarding frames from local to server
    pub async fn run(&self, ws_id: &str) {
